"""Script to train Time-to-Lane-Change (TTLC) Predictors with k-means friendly spaces"""
import os
import torch
import numpy as np
...
from modules import ttlc, ttlcSOTA
...
def main():
    ...
    for model_name, model_arguments in params.models.items():
        ...
        for train_ind, val_ind in skf.split(X_train, y_train):
            ...
            ### Depending on the model name, build the right model ###
            if model_arguments[1] == 'rnn':
                model_tmp = ttlcSOTA.rnnTTLC(name=model_name + '_kFold{}'.format(i),
                                             loss=params.loss,
                                             num_clusters=params.num_clusters,
                                             num_training_data=X_train.shape[0],
                                             **model_arguments[0]).double()
            elif model_arguments[1] == 'transformer':
                model_tmp = ttlcSOTA.transformerTTLC(name=model_name + '_kFold{}'.format(i),
                                                     loss=params.loss,
                                                     num_clusters=params.num_clusters,
                                                     num_training_data=X_train.shape[0],
                                                     **model_arguments[0]).double()
            else:
                model_tmp = ttlc.cnnTTLC(name=model_name + '_kFold{}'.format(i),
            ...
            if bool_train:
                ...
                elif model_arguments[1] == 'rnn' or model_arguments[1] == 'transformer':
                    X_train_tmp = X_train.permute(0, 2, 1)
                else:
                    X_train_tmp = X_train
                ...
                # Add the callback to save the model at certain epochs
                ...
                # Check whether to add the kMeansFriendly Callback
                if params.loss == 'k-means-friendly':
                    kMeansFriendly = th.KMeansFriendly(
                        train_loader=train_loader,
                        num_pretrain_epochs=params.pretrain_epochs,
                        lambda_kmeans=params.lambda_kmeans,
                    )
                    callbacks.append(kMeansFriendly)
                ...
                elif params.loss == 'k-means-friendly':
                    compute_loss = th.compute_kmeans_friendly_loss()
                ...
                if bool_save:
                    ...
                    torch.save(model_tmp,
                                PATH)
                    # Save the cluster centres from k-means training
                    if params.loss == 'k-means-friendly':
                        PATH = os.path.join(path_weights,
                                            file_save_name + '_' + model_tmp.name +
                                            'clusters.mat')
                        model_tmp.save_cluster_centers(PATH=PATH)
    ...
    if bool_save:
        ...
        for model_name, model_tmp in models.items():
            ...
            elif 'lstm' in model_name or 'gru' in model_name or 'Trans' in model_name:
                X_test_tmp = X_test.permute(0, 2, 1)
            else:
                X_test_tmp = X_test
