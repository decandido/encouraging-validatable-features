...
################################################################################
## Parameters for the different ML architectures                              ##
################################################################################
...
d_model = 50  # model-dimension
q = 16  # Query/key matrix dimension
v = 16  # value matrix dimension
h = 8  # number of heads
N = 3  # number of encoder blocks
...
# loss = nn.NLLLoss
loss = 'k-means-friendly'
# Define how many clusters we want for the k-means-friendly space
num_clusters = num_classes
# Some options for the k-means-friendly space loss
# How many epochs should the loss function concentrate only on the cross
# entropy term, before turning on the clustering loss?
pretrain_epochs = 1
# What is the trade-off parameter for the clustering loss?
lambda_kmeans = 1
# Preload cluster center values (Default: None)
# If this is set to None, no clusters will be initalised
path_load_M = None # Otherwise a path to cluster centres
...
## LSTM design                                                                ##
################################################################################
lstm_args = dict(input_dim=len(features),
...
## GRU design                                                                 ##
################################################################################
gru_args = dict(input_dim=len(features),
                 hidden_dim=embedding_dim,
                 type='gru',
                 output_layer=output_layer,
                 in_classifier=in_classifier,
...
## Transformer design                                                         ##
################################################################################
tran_args = dict(d_input=len(features),
                 d_model=d_model,
                 q=q,
                 v=v,
                 h=h,
                 N=N,
                 length_timeseries=frame_rate*seconds_per_bin,
...
models = dict(
              ...
              lstmRNN=[lstm_args, 'rnn'],
              gruRNN=[gru_args, 'rnn'],
              gatedTrans=[tran_args, 'transformer'],
)
...
for name, item in models.items():
    assert item[1] == 'standard' or \
           ...
           item[1] == 'rnn' or \
           item[1] == 'transformer' or \
           item[1] == 'dense', 'The model must be built has a standard CNN, ' \
                               'a multi-channel CNN, a RNN, a transformer, or a dense NN'
